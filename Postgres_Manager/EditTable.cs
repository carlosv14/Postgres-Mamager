...
namespace Postgres_Manager
{
    public partial class EditTable : Form
    {
        ...
        public EditTable(NpgsqlConnection conn, string selectedTable,RichTextBox t)
        {
            ...
            rows = columns.Rows.Count+1;
            for (int i = 0; i < columns.Rows.Count; i++)
            {
                dataGridView1.Rows.Add();
                dataGridView1.Rows[dataGridView1.Rows.Count - 2].Cells["PK"].Value = false;
                dataGridView1.Rows[dataGridView1.Rows.Count - 2].Cells["NOMBRE"].Value = columns.Rows[i][0].ToString();
                dataGridView1.Rows[dataGridView1.Rows.Count - 2].Cells["TIPO"].Value = columns.Rows[i][1].ToString();
            }
            pks = pc.exec_Sql(
            ...
            for (int i = 0; i < columns.Rows.Count; i++)
            {
                for (int j = 0; j < pks.Rows.Count; j++)
                {
                    if (dataGridView1.Rows[i].Cells["NOMBRE"].Value.ToString() == pks.Rows[j][0].ToString())
                        dataGridView1.Rows[i].Cells["PK"].Value = true;
                }
            }
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (tabControl1.SelectedIndex == 1 || tabControl1.SelectedIndex == 2)
            {
                ...
                richTextBox1.Text += "--------MODIFICATIONS---------\n";
                
                if (dataGridView1.Rows.Count > rows)
                {
                    for(int i = rows-1; i< dataGridView1.Rows.Count-1;i++)
                        mods += "ALTER TABLE " + selectedTable + " ADD COLUMN " +
                                dataGridView1.Rows[i].Cells["NOMBRE"].Value.ToString() + " " +
                                dataGridView1.Rows[i].Cells["TIPO"].Value.ToString() +";\n";
                        richTextBox1.Text += mods;
                    
                   
                }else if (dataGridView1.Rows.Count < rows)
                {
                    mods = "";
                    for (int i = 0; i < delete.Count; i++)
                        mods += "ALTER TABLE " + selectedTable + " DROP COLUMN " +
                                delete.ElementAt(i).ToString() + ";\n";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            pc.exec_Sql(mods, conn, t);

            if (datarows > dataGridView2.Rows.Count || datarows< dataGridView2.Rows.Count)
            {
                sql = "delete from " + selectedTable;
                pc.exec_Sql(sql, conn, t);
                ...
                for (int i = 0; i < dataGridView2.Rows.Count - 1; i++)
                {
                    ...
                    {
                        sql +="'"+dataGridView2.Rows[i].Cells[j].Value.ToString()+"'";
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                delete.Add(dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["NOMBRE"].Value.ToString());
